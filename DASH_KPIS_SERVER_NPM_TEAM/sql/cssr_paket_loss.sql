...
SELECT
    ...
    COALESCE(h2g.time, e2g.TIME, epl.TIME, hpl.TIME)   AS TIME,
...
LEFT JOIN ERICSSON_2G_KPI  e2g ON h2g.date = e2g.DATE AND h2g.time = e2g.TIME
LEFT JOIN ERICSSON_PACKET_LOSS epl ON COALESCE(h2g.date, e2g.DATE) = epl.DATE AND COALESCE(h2g.time, e2g.TIME) = epl.TIME
LEFT JOIN HUAWEI_PACKET_LOSS   hpl ON COALESCE(h2g.date, e2g.DATE) = hpl.DATE AND COALESCE(h2g.time, e2g.TIME) = hpl.TIME
